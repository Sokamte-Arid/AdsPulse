...
router.post('/', auth, async (req, res) => {
  try {
    const {
      name, objective, startDate, endDate, totalBudget, currency,
      platforms, tags, notes, creative
    } = req.body;
    ...
    const campaign = await Campaign.create({
      ...
      platforms, tags, notes, creative,
      userId: req.user._id,
      status: 'draft',   // always start as draft — never trust client
    });
...
router.put('/:id', auth, async (req, res) => {
  try {
    const campaign = await Campaign.findOneAndUpdate(
      ...
      (({ name, objective, startDate, endDate, totalBudget, currency, platforms, tags, notes, creative, status }) =>
        ({ name, objective, startDate, endDate, totalBudget, currency, platforms, tags, notes, creative, status, updatedAt: new Date() })
      )(req.body),
...
router.patch('/:id/platforms/:platform/budget', auth, requireRole('manager','admin'), async (req, res) => {
  try {
...
router.post('/:id/push', auth, requireRole('manager','admin'), async (req, res) => {
  try {
    const campaign = await Campaign.findOne({ _id: req.params.id, userId: req.user._id });
    if (!campaign) return res.status(404).json({ message: 'Campaign not found' });
