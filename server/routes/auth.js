...
if (!process.env.JWT_SECRET)     throw new Error('[FATAL] JWT_SECRET env var is not set.');
if (!process.env.JWT_2FA_SECRET) throw new Error('[FATAL] JWT_2FA_SECRET env var is not set.');
const JWT_SECRET     = process.env.JWT_SECRET;
const JWT_2FA_SECRET = process.env.JWT_2FA_SECRET;

// ── Token helpers ─────────────────────────────────────────────────────────────
const signToken     = (userId, role = 'manager') => jwt.sign({ userId, role }, JWT_SECRET, { expiresIn: '7d' });
...
// ── Email sender ──────────────────────────────────────────────────────────────
const { sendEmail: _sendEmail } = require('../utils/email');
...
router.post('/verify-2fa', async (req, res) => {
  try {
    ...
    if (user.twoFactorMethod === 'totp') {
      ...
      valid = user.emailOTP.length === code.trim().length &&
        crypto.timingSafeEqual(Buffer.from(user.emailOTP), Buffer.from(code.trim()));
