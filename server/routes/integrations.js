...
router.post('/:platform/connect', auth, async (req, res) => {
  ...
  try {
    ...
    if (platform === 'meta') {
      ...
      // Instagram uses Meta OAuth token — auto-connect if Meta is connected
      const metaConn = user.connectedPlatforms.find(p => p.platform === 'meta' && p.status === 'connected');
      if (!metaConn) return res.status(400).json({ message: 'Connect Meta first — Instagram uses the same account.' });

      // Get Instagram Business Account linked to the Facebook Page
      try {
        const pagesRes = await axios.get('https://graph.facebook.com/v19.0/me/accounts', {
          params: { fields: 'id,name,instagram_business_account', access_token: metaConn.accessToken }
        });
        const page = (pagesRes.data?.data || []).find(p => p.instagram_business_account);
        if (!page) return res.status(400).json({ message: 'No Instagram Business account found linked to your Facebook Page.' });

        const igId   = page.instagram_business_account.id;
        const igRes  = await axios.get(`https://graph.facebook.com/v19.0/${igId}`, {
          params: { fields: 'id,name,username,followers_count,profile_picture_url', access_token: metaConn.accessToken }
        });

        accountInfo = {
          accountId:   igId,
          accountName: `@${igRes.data.username || igRes.data.name}`,
        ...
        return res.status(400).json({ message: `Instagram connection failed: ${igErr.response?.data?.error?.message || igErr.message}` });
      }
    }
    else {
...
router.post('/:platform/sync', auth, async (req, res) => {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════
      ...
      console.log(`[SYNC Instagram] Account ID: ${igAccountId}`);

      try {
        // Fetch Instagram ads (through Meta Graph API using Instagram account)
        const mediaRes = await axios.get(`https://graph.facebook.com/v19.0/${igAccountId}/media`, {
          params: { fields: 'id,caption,media_type,thumbnail_url,permalink,timestamp,like_count,comments_count', limit: 50, access_token: conn.accessToken },
          timeout: 15000
        });

        const posts = mediaRes.data?.data || [];
        console.log(`[SYNC Instagram] Found ${posts.length} posts`);

        // Also fetch Instagram ads via Meta Ads API
        const metaConn = user.connectedPlatforms.find(p => p.platform === 'meta');
        if (metaConn?.accountId && metaConn?.accessToken) {
          try {
            const igCampRes = await axios.get(`https://graph.facebook.com/v19.0/${metaConn.accountId}/campaigns`, {
              params: {
                fields: 'id,name,status,effective_status,objective,daily_budget,lifetime_budget',
                effective_status: JSON.stringify(['ACTIVE','PAUSED']),
                limit: 100,
                access_token: metaConn.accessToken
              },
              timeout: 15000
            });

            const igCampaigns = (igCampRes.data?.data || []).filter(c =>
              c.objective === 'OUTCOME_ENGAGEMENT' || c.objective === 'POST_ENGAGEMENT' ||
              c.objective === 'BRAND_AWARENESS' || c.objective === 'REACH'
            );

            console.log(`[SYNC Instagram] Found ${igCampaigns.length} Instagram-related campaigns`);

            for (const mc of igCampaigns) {
              const budget = parseFloat(mc.daily_budget || mc.lifetime_budget || 0) / 100;
              ...
              try {
                const insRes = await axios.get(`https://graph.facebook.com/v19.0/${mc.id}/insights`, {
                  params: { fields: 'spend,impressions,cpm,clicks,ctr,cpc,reach', date_preset: 'last_30d', access_token: metaConn.accessToken },
                  timeout: 8000
                });
                const ins = insRes.data?.data?.[0];
                if (ins) {
                  metrics = {
                    amountSpent: parseFloat(ins.spend || 0),
                    ...
                    conversions: 0, addToCart: 0
                  };
                }
              } catch {}

              const existing = await Campaign.findOne({ userId: req.user._id, 'platforms.platformCampaignId': mc.id + '_ig' });
              if (existing) {
                const pIdx = existing.platforms.findIndex(p => p.platformCampaignId === mc.id + '_ig');
                if (pIdx >= 0) { existing.platforms[pIdx].metrics = metrics; existing.platforms[pIdx].status = mappedStatus; await existing.save(); updated++; }
              } else {
                await Campaign.create({
                  userId: req.user._id, name: mc.name + ' (Instagram)',
                  objective: 'engagement', status: mappedStatus,
                  totalBudget: budget, currency: 'USD',
                  platforms: [{ platform: 'instagram', platformCampaignId: mc.id + '_ig', status: mappedStatus, budget, metrics }],
                  tags: ['imported', 'instagram']
                });
                imported++;
              }
            }
          } catch (err) {
            console.log(`[SYNC Instagram] Ads fetch warning: ${err.message}`);
          }
        }

        console.log(`[SYNC Instagram] ✅ Done — ${imported} imported, ${updated} updated`);
      } catch (igErr) {
        const msg = igErr.response?.data?.error?.message || igErr.message;
        console.log(`[SYNC Instagram] ❌ Error: ${msg}`);
        return res.status(400).json({ message: `Instagram sync failed: ${msg}` });
      }
    }
