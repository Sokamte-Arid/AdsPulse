  ...
  for (const mc of campaigns) {
    ...
    try {
      ...
      if (ins) {
        ...
        // ── Video metrics ─────────────────────────────────────────────────
        const findAction = (arr, type) => parseInt(arr?.find(a => a.action_type === type)?.value || 0);
        metrics.video3SecViews = findAction(ins.video_3_sec_watched_actions,   'video_view');
        metrics.videoP25       = findAction(ins.video_p25_watched_actions,     'video_view');
        metrics.videoP50       = findAction(ins.video_p50_watched_actions,     'video_view');
        metrics.videoP75       = findAction(ins.video_p75_watched_actions,     'video_view');
        metrics.videoP100      = findAction(ins.video_p100_watched_actions,    'video_view');
        metrics.thruPlays      = findAction(ins.video_thruplay_watched_actions,'video_view');
        metrics.avgWatchTime   = parseFloat(ins.video_avg_time_watched_actions?.[0]?.value || 0);
        metrics.videoViews     = metrics.video3SecViews; // 3-sec is the standard "view"
        // Derived
        metrics.cpv          = metrics.videoViews > 0 ? parseFloat((metrics.amountSpent / metrics.videoViews).toFixed(4)) : 0;
        metrics.videoViewRate = metrics.impressions > 0 ? parseFloat(((metrics.videoViews / metrics.impressions) * 100).toFixed(2)) : 0;
        metrics.completionRate = metrics.videoViews > 0 ? parseFloat(((metrics.videoP100 / metrics.videoViews) * 100).toFixed(2)) : 0;
