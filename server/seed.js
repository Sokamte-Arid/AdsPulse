...
async function seed(){
  ...
  const orgSchema = new mongoose.Schema({
    ...
    members: [{ userId: mongoose.Schema.Types.ObjectId, email: String, name: String,
      role: String, status: String, joinedAt: Date }],
    plan: String, settings: Object,
  }, { timestamps: true });
  const OrgModel = mongoose.models.Organization || mongoose.model('Organization', orgSchema);
  await OrgModel.deleteMany({ ownerId: user._id });
  await OrgModel.create({
    name: 'AdsPulse Demo Agency',
    ownerId: user._id,
    plan: 'pro',
    members: [{
      userId:   user._id,
  ...
  console.log('🏢 Created: AdsPulse Demo Agency (owner: demo@adspulse.com)');
