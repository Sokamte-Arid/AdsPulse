...
import { Bolt, Building, Check, Close, CreditCard, Receipt, TrendingUp } from '../components/shared/Icons.js';
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 6 }}>
        <div style={{
          ...
          {plan.id === 'starter' ? TrendingUp : plan.id === 'pro' ? Bolt : Building}
        </div>
      ...
      >
        {isCurrentPlan ? <><Check size={14}/> Active</> : `Upgrade to ${plan.name}`}
      </button>
    </div>
  );
}
    ...
    <Layout>
      ...
      {toast && (
        <div style={{
          ...
          <span>{toast}</span>
          <button onClick={() => setToast('')} style={{ background: 'none', border: 'none', color: 'var(--text-faint)', cursor: 'pointer', marginLeft: 'auto', fontSize: 16, padding: 0 }}><Close size={14}/></button>
        </div>
      )}
      ...
      <div style={{
        ...
        <span style={{ fontSize: 20, flexShrink: 0 }}><CreditCard size={14}/></span>
        <div>
