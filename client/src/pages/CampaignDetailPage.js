...
import { Check, CheckCircle, Close, Cursor, DollarSign, DynIcon, Edit, Eye, Planner, Rocket, Target, XCircle } from '../components/shared/Icons.js';;
  ...
  return (
    <Layout>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:14 }}>
          <div style={{ flex:1 }}>
            ...
            <div style={{ display:'flex', gap:14, flexWrap:'wrap', fontSize:13, color:'var(--text-muted)' }}>
              <span>{obj?.icon && React.createElement(obj.icon, { size: 14 })} {obj?.label}</span>
              {campaign.startDate && <span><Planner size={14}/> {new Date(campaign.startDate).toLocaleDateString()} – {campaign.endDate ? new Date(campaign.endDate).toLocaleDateString() : 'Ongoing'}</span>}
              <span><DollarSign size={14}/> {campaign.currency} {(campaign.totalBudget || 0).toLocaleString()}</span>
            </div>
          </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:14, marginBottom:28 }}>
        {[
          { label:'Amount Spent',  value:formatKPI(totalSpent,'currency'),      sub:`of $${(campaign.totalBudget||0).toLocaleString()} budget`, icon:DollarSign, color:'#a855f7' },
          { label:'Impressions',   value:formatKPI(totalImpressions,'number'),   sub:`across ${campaign.platforms?.length} platforms`,          icon:Eye,  color:'#3b82f6' },
          { label:'Total Clicks',  value:formatKPI(totalClicks,'number'),        sub:`CTR ${totalImpressions>0?((totalClicks/totalImpressions)*100).toFixed(2):0}%`, icon:Cursor, color:'#10b981' },
          { label:'Conversions',   value:formatKPI(totalConversions,'number'),   sub:`CPA $${totalConversions>0?(totalSpent/totalConversions).toFixed(2):'0'}`,       icon:Target, color:'#f59e0b' },
        ].map(stat => (
          <div key={stat.label} className="glass-card" style={{ padding:'18px 20px' }}>
            <div style={{ fontSize:20, marginBottom:8 }}><DynIcon icon={stat.icon} size={14}/></div>
                    ...
                    <td style={{ padding:'14px 12px' }}>
                      {isEditing ? (
                        ...
                        <div style={{ cursor:'pointer' }} onClick={() => { setEditingBudget(p.platform); setBudgetValue(p.budget); setBudgetType(p.budgetType || 'daily'); }}>
                          ...
                          <div style={{ fontSize:10, color:'var(--purple-light)', marginTop:2 }}><Edit size={14}/>️ Click to edit</div>
                        </div>
                      )}
                    </td>
                    {/* Metrics */}
