...
import { AlertTriangle, Campaigns, CheckCircle, Clipboard, DollarSign, Planner, Refresh, Schedules, Settings, Trash, XCircle } from '../components/shared/Icons.js';
...
const TYPE_CONFIG = {
  ...
  budget_change: { icon:DollarSign, label:'Budget Change', color:'#7c3aed' },
  recurring:     { icon:Refresh, label:'Recurring',     color:'#3b82f6' },
};
    ...
    <Layout>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))', gap:14, marginBottom:28 }}>
        {[
          ...
          { label:'Executed', value:executed, icon:CheckCircle, color:'#16a34a' },
          { label:'Failed',   value:failed,   icon:XCircle, color:'#ef4444' },
          { label:'Total',    value:schedules.length, icon:Clipboard, color:'#7c3aed' },
        ].map(s => (
      ...
      <div style={{ padding:'16px 20px', borderRadius:12, background:'rgba(124,58,237,0.07)', border:'1px solid rgba(124,58,237,0.18)', marginBottom:24, display:'flex', gap:14 }}>
        <span style={{ fontSize:22, flexShrink:0 }}><Settings size={14}/>️</span>
      ...
      {loading ? (
        ...
        <div className="glass-card" style={{ padding:48, textAlign:'center' }}>
          <div style={{ fontSize:48, marginBottom:16 }}><Schedules size={40}/></div>
          ...
          {Object.entries(groups).filter(([,items]) => items.length > 0).map(([group, items]) => (
            <div key={group}>
              <div style={{ fontSize:12, fontWeight:700, color: group==='Overdue'?'#ef4444':'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:10, display:'flex', alignItems:'center', gap:8 }}>
                {group==='Overdue' && <AlertTriangle size={14}/>} {group} ({items.length})
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
    ...
    <div className="glass-card" style={{ padding:'16px 20px', display:'flex', gap:14, alignItems:'flex-start', flexWrap:'wrap',
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:4 }}>
          ...
          {isPast && <span style={{ fontSize:11, fontWeight:700, color:'#ef4444' }}><AlertTriangle size={14}/>️ Overdue</span>}
        </div>
        ...
        <div style={{ display:'flex', gap:14, flexWrap:'wrap', fontSize:12, color:'var(--text-muted)' }}>
          {campaign?.name && (
            <button onClick={() => navigate(`/campaigns/${campaign._id || campaign}`)}
              ...
              <Campaigns size={14}/> {campaign.name}
            </button>
          )}
          <span><Planner size={14}/> {formatDate(s.scheduledAt)}</span>
          {s.status === 'pending' && (
            <span style={{ color: isPast?'#ef4444':'#3b82f6', fontWeight:600 }}>
              {timeUntil(s.scheduledAt)}
            </span>
          )}
          {s.recurrence?.frequency && <span style={{ color:'var(--purple-light)' }}><Refresh size={14}/> {s.recurrence.frequency}</span>}
          {s.action?.budget && <span><DollarSign size={14}/> → ${s.action.budget} {s.action.budgetType}</span>}
          {s.executedAt && <span><CheckCircle size={14}/> Ran {formatDate(s.executedAt)}</span>}
          {s.errorMessage && <span style={{ color:'#ef4444' }}><AlertTriangle size={14}/>️ {s.errorMessage}</span>}
        </div>
