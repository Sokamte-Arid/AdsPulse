...
import { AlertTriangle, Check, CheckCircle, Close, DollarSign, DynIcon, Edit, Planner, Refresh, Schedules, Trash, XCircle } from '../shared/Icons.js';;
...
const SCHEDULE_TYPES = [
  ...
  { id:'budget_change', icon:DollarSign, label:'Change Budget',   desc:'Update campaign budget at a specific time' },
  { id:'recurring',     icon:Refresh, label:'Recurring',       desc:'Repeat an action on a schedule' },
];
      ...
      <div className="glass-card" style={{ padding:32, maxWidth:520, width:'100%', maxHeight:'90vh', overflowY:'auto' }}
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
          <h3 style={{ fontSize:18, fontWeight:800, color:'var(--text-primary)', margin:0 }}>
            {isEdit ? <><Edit size={14}/> Edit Schedule</> : <><Schedules size={14}/> Add Schedule</>}
          </h3>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
            {SCHEDULE_TYPES.map(t => (
              <div key={t.id} onClick={() => update('type', t.id)}
                ...
                <div style={{ fontSize:18, marginBottom:4 }}><DynIcon icon={t.icon} size={14}/></div>
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button className="btn-primary" onClick={handleSave} disabled={saving} style={{ flex:2, justifyContent:'center' }}>
            {saving ? '⏳ Saving...' : isEdit ? <><Check size={14}/> Update Schedule</> : <><Check size={14}/> Add Schedule</>}
          </button>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <div>
          <h3 style={{ fontSize:17, fontWeight:700, color:'var(--text-primary)', margin:'0 0 4px' }}>
            <Schedules size={14}/> Campaign Scheduling
          </h3>
      ...
      {loading ? (
        ...
        <div style={{ padding:'36px 20px', borderRadius:14, border:'2px dashed var(--border-subtle)', background:'var(--bg-elevated)', textAlign:'center' }}>
          <div style={{ fontSize:42, marginBottom:12 }}><Schedules size={36}/></div>
                    ...
                    <div key={s._id} style={{ padding:'16px 18px', borderRadius:12, background:'var(--bg-card)', border:'1px solid var(--border-subtle)', display:'flex', gap:14, alignItems:'flex-start', flexWrap:'wrap' }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ display:'flex', gap:12, flexWrap:'wrap', fontSize:12, color:'var(--text-muted)' }}>
                          <span><Planner size={14}/> {formatScheduleDate(s.scheduledAt)}</span>
                          <span style={{ color: isPast ? '#ef4444' : '#3b82f6', fontWeight:600 }}>
                            {timeUntil(s.scheduledAt)}
                          </span>
                          {s.recurrence?.frequency && (
                            <span style={{ color:'var(--purple-light)' }}><Refresh size={14}/> {s.recurrence.frequency}</span>
                          )}
                          {s.action?.budget && (
                            <span><DollarSign size={14}/> → ${s.action.budget} {s.action.budgetType} ({s.action.platform || 'all'})</span>
                          )}
                        </div>
