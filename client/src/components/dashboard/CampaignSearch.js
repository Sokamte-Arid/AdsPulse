...
import { Close, Search } from '../shared/Icons.js';
          ...
          <div style={{ position:'absolute', right:10, top:'50%', transform:'translateY(-50%)' }}>
            {loading ? (
              ...
              <button onClick={() => { setQuery(''); setResults([]); setOpen(false); inputRef.current?.focus(); }}
                style={{ background:'none', border:'none', color:'var(--text-faint)', cursor:'pointer', display:'flex', padding:2, fontSize:14 }}><Close size={14}/></button>
            )}
          </div>
        )}
      </div>
        ...
        <div style={{
          ...
          {results.length === 0 && !loading ? (
            <div style={{ padding:'20px 16px', textAlign:'center', fontSize:13, color:'var(--text-faint)' }}>
              <div style={{ fontSize:28, marginBottom:8 }}><Search size={14}/></div>
