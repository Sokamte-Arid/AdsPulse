...
import { Connect, Star } from '../shared/Icons.js';
...
export default function PlatformPerformanceChart() {
  ...
  // Check if platforms have actual metric data
  const hasRealData = data.some(d => d.impressions > 0 || d.amountSpent > 0 || d.totalClicks > 0);
    ...
    <div className="glass-card" style={{ padding:24 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        <div>
          <h3 style={{ fontSize:17, fontWeight:700, color:'var(--text-primary)', margin:'0 0 4px' }}>
            <Star size={14}/> Platform Performance
          </h3>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div style={{ fontSize:40, marginBottom:12 }}><Star size={36}/></div>
          <div style={{ fontSize:15, fontWeight:700, color:'var(--text-primary)', marginBottom:8 }}>
            No Platform Data Yet
          </div>
          <div style={{ fontSize:13, color:'var(--text-muted)', lineHeight:1.7, maxWidth:340, margin:'0 auto 20px' }}>
No campaign data found. Create a campaign or sync your connected platforms to see performance rankings. Once your campaigns have run and collected data, rankings will appear here automatically.
          </div>
          <div style={{ display:'flex', gap:10, justifyContent:'center', flexWrap:'wrap' }}>
            ...
            <button className="btn-secondary" onClick={() => navigate('/connect')} style={{ fontSize:12 }}>
              <Connect size={14}/> Connect Platform
            </button>
          </div>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          {!hasRealData && (
            <div style={{ padding:'10px 14px', borderRadius:8, background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.25)', fontSize:12, color:'#f59e0b', marginBottom:4, lineHeight:1.6 }}>
              Campaigns found but no spend data yet. Sync your platforms or wait for campaigns to start running — rankings will update automatically.
            </div>
          )}
          {sorted.map((item, idx) => {
            ...
            const hasMetrics = item.impressions > 0 || item.amountSpent > 0 || item.totalClicks > 0 || item.campaignCount > 0;
            return (
              <div key={item.platform} style={{ display:'flex', alignItems:'center', gap:14, opacity: hasMetrics ? 1 : 0.5 }}>
                {/* Rank */}
                ...
                {/* Pending indicator */}
                {!hasMetrics && <div style={{ fontSize:10, color:'var(--text-faint)', flexShrink:0, whiteSpace:'nowrap' }}>Sync needed</div>}
                {/* Value */}
